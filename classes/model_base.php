...
Abstract Class Model_Base {
    ...
    public function __construct($select = false, $fields = false) {
        // объект бд коннекта
        global $dbObject;
        $this->db = $dbObject;
        ...
        $this->table = is_null($this->getTableName()) ? $tableName : $this->getTableName();
        ...
        if($sql) {
            $this->_getResult("SELECT $queryFields FROM $this->table" . $sql);
        }else{
            $this->_getResult("SELECT $queryFields FROM $this->table");
        }
    }
